...
public class FindPosition extends Conditional {
	...
	public State execute() {
		try{
			...
			if (building == UnitType.Terran_Factory && (((JohnDoe)this.handler).academy == 0 ||
					((JohnDoe)this.handler).factory >= ((JohnDoe)this.handler).CCs.size()+1) ) {
				return State.FAILURE;
			}
			//Decorator for engineering bay. Only after have built the factory and only 1
			if (building == UnitType.Terran_Engineering_Bay &&
					((JohnDoe)this.handler).barracks < 2 || ((JohnDoe)this.handler).bay > 0) {
				return State.FAILURE;
			}
